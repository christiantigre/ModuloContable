...
$insertasientoconcepto = "INSERT INTO `condata`.`num_asientos` (`idnum_asientos` ,`fecha` ,`concepto` , `t_bl_inicial_idt_bl_inicial`,"
        ...
        . "('" . $incremento . "' , '" . $date . "', '" . trim($concepto) . "', '" . $maxbalancedato . "','" . $asiento_num . "','" . $month . "','" . $year . "','" . $saldodebe . "','" . $saldohaber . "');";
...
if (mysqli_connect_errno()) {
    ...
    $accion = "/ SECRETARIA / [ERROR] AL INSERTAR EL ASIENTO N# " . $asiento_num;
    generaLogs($user, $accion);
} else {
    print_r("Guardado con exito...");

    $accion = "/ CONTABILIDAD / GUARDO EL ASIENTO N# " . $asiento_num;
    generaLogs($user, $accion);
}
