...
function newAlert(type, message) {
    ...
    $(".alert-message").delay(9000).fadeOut("slow", function () {
        $(this).remove();
    });
}


function compr_evento_level() {
    ...
    if (counter > 1) {
        ...
        $.post("./scriptsPHP/archivo.php", {"texto": miVariableJS, "campo": "cod_cuenta"},
        function (respuesta) {
            ...
            fcarg_grupo_level(var3);
        });
    } else {
        var miVariableJS = $("#cod_cuenta").val();
        $.post("./scriptsPHP/archivo.php", {"texto": miVariableJS, "campo": "nombre_cuenta_plan"},
        function (respuesta) {
            var elem = respuesta.split('_');
            var1 = elem[0];
            var2 = elem[1];
    ...
    function fcarg_grupo_level(var3)
    {
        $.post("./scriptsPHP/archivogruponom.php", {"texto": var3},
        function (respuestag) {
...
function aux_save_level_asini()
{
    $('#form').submit(function (msg) {
        $.post("./scriptsPHP/scriptupdata_ej.php", $(this).serialize(), function (data) {
            alert(data);
            newAlert("success", " " + data);
            $("#textarea_as").attr("value", "");
...
function aux_save_level_asup()
{
    $('#form').submit(function (msg) {
        $.post("./scriptsPHP/scriptupdata_lib.php", $(this).serialize(), function (data) {
            alert(data);
            newAlert("success", " " + data);
            $("#textarea_as").attr("value", "");
            $("#camposumadebet").attr("value", "");
...
function imp_assin_order_by() {
    var tipo=document.getElementById("lista_ordenar").value;
    var orden=document.getElementById("ac_dc").value;
    var cadena = tipo + ' '+orden;
    window.open('./documentos/impresiones/order.php?cadena=' + cadena+"&idlogeo=");
}
