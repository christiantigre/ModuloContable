...
if ($month==="0") {
    ...
    $mes = "SELECT * FROM mes where mes_id ='" . $month . "'";
    $resultmes = mysqli_query($db, $mes) or trigger_error("Query Failed! SQL: $mes- Error: " . mysqli_error($db), E_USER_ERROR);
    if ($resultmes) {
...
$pdf->Cell(0, 7, ' Por el mes de ' . translateMonth($datomes) . ' del ' . $yearcont, 0, 1, 'C');
...
if ($month === "0") {
    ...
    $sql_ingresos = "SELECT codigo,cuenta,total FROM estadoresultados where codigo between '4.' and '4.99.99.99.' and mes='" . $month . "' ORDER BY codigo ASC";
}
...
if ($month2 === "0") {
    ...
    $sql_gastos = "SELECT codigo,cuenta,total FROM estadoresultados where codigo between '5.' and '5.99.99.99.' and mes = '" . $month2 . "' ORDER BY codigo ASC";
}
