...
class backupmysql {
    ...
    private $dir = 'C:\Users\ANDRES/';  //folder to store ZIP archive with SQL backup
    public $lang = [];  //array with texts from "lang_...json"

    // receives languuage indice $lang, and folder-path that stores backup

    function __construct($lang = 'en', $dir = 'C:\Users\ANDRES/') {
        $this->setLang($lang);
        if ($dir != '')
            $this->dir = trim($dir, '/') . '/';
    }
    ...
    private function setLang($lang) {
        if (file_exists('lang_' . $lang . '.json'))
            $this->lang = json_decode(file_get_contents('lang_' . $lang . '.json'), true);
        else if (file_exists('lang_en.json'))
            $this->lang = json_decode(file_get_contents('lang_en.json'), true);
    ...
    public function getTables() {
        ...
        $nr = $this->num_rows;
        for ($i = 0; $i < $nr; $i++)
            $re[] = $resql[$i]['Tables_in_' . $this->mysql['dbname']];
        return $re;
    }
    ...
    public function getListTables() {
        ...
        $accion = "/ BACKUPS / acceso / seleccion de respaldos";
        generaLogs($this->mysql['user'], $accion);

        //if bot error, set form, else add error in $re['er']
        if (!$this->error) {
            $nr = count($tables); // 
            $re['f'] = '<form action="' . $_SERVER['REQUEST_URI'] . '" method="post" id="frm_cht" name="frm_cht">'
                    . '<h3>' . $this->langTxt('msg_select_tbles') . '</h3>'
                    . '<label id="ch_all"><input onclick="check1()" id="todos"  type="checkbox" readonly="readonly" checked onclick="javascript: return false;" >' . sprintf($this->langTxt('msg_select_all'), $nr) . '</label>'
                    . '<br><div  class="ch_tables">';

            for ($i = 0; $i < $nr; $i++) {
                if ($i > 0 && ($i % 12) == 0)
                    $re['f'] .='</div>'
                            . '<div class="ch_tables"><tr>';  //close Div and open the next
                $re['f'] .= '<td><label><input type="checkbox" name="tables[]" value="' . $tables[$i] . '" readonly="readonly" checked onclick="javascript: return false;" > ' . $tables[$i] . ' </label></td></tr>';
            }
            $re['f'] .='</table></div>'
                    . '<br>';  //close last .ch_tables
            $re['f'] = str_ireplace('<div class="ch_tables"></div>', '', $re['f']) . '<input class="btn btn-success" type="submit" value="' . $this->langTxt('msg_backup') . '">'
                    . '</form>';
        } else
            $re['er'] = $this->error;
        return $re;
...
SET AUTOCOMMIT = 1; ' . PHP_EOL;
    ...
    public function saveBkZip($tables) {
        if ($this->dir == '' || is_writable($this->dir)) {
            $file_sql = 'backup-' . $this->mysql['dbname'] . '-' . date('Y-m-d') . '@' . date('h.i.s') . '.sql';  //name of the SQL file with backup put in ZIP
        ...
        $accion = "/ BACKUPS / respaldó / creó respaldo fecha :" . date('Y-m-d') . '@' . date('h.i.s') . '.sql';
        generaLogs($this->mysql['user'], $accion);
    ...
    public function getListZip() {
        $re = '<form action="' . $_SERVER['REQUEST_URI'] . '" method="post" id="frm_zip"><h3>' . $this->langTxt('msg_select_file') . '</h3>'
                . '<input type="hidden" id="dir_bk" value="' . $this->dir . '">'
                . '<input type="hidden" name="bmd_zip" value="restore">';  //hidden fields used when request to #get_file / #del_file
        ...
        for ($i = 0; $i < $nr; $i++) {
            ...
            $re .= '<table  class="table table-striped table-bordered table-hover" id="dataTables-example">'
                    . '<tr><label><input type="radio" name="file" value="' . $file . '" class="zip_files"> ' . $file . ' (<em>' . $size . '</em>)</label>'
                    . '</tr></table>';
        }
//        $btn = ($nr > 0) ? '<input class="btn btn-success" type="button" value="' . $this->langTxt('msg_restore_bk') . '" id="res_file"> '
//                . '<input class="btn btn-primary" type="button" value="' . $this->langTxt('msg_get_file') . '" id="get_file"> '
//                . '<input class="btn btn-danger" type="button" value="' . $this->langTxt('msg_del_file') . '" id="del_file">' : $this->langTxt('msg_nozip');
        ...
        if ($zip->open($this->dir . $zp) === TRUE) {
            ...
            if (floor(strlen($sql) / (1024 * 1024)) < 50) {
                ...
                if (file_put_contents($this->dir . 'temp.sql', $sql)) {
                    ...
                    $re = sprintf($this->langTxt('er_write_in'), $this->dir);
            } else
                $this->langTxt('er_exec');
        } else
            $re = sprintf($this->langTxt('er_open'), $zp);

        $accion = "/ BACKUPS / restauró / restauró dase de datos";
        generaLogs($this->mysql['user'], $accion);
    ...
    public function getZipFile($zip) {
        if (file_exists($this->dir . $zip)) {
            ...
            return sprintf($this->langTxt('er_file'), $zip);
    }

    //delete $file from $dir
    public function delFile($file) {
        if (@unlink($this->dir . $file))
            return $this->langTxt('ok_delete');
        else
            return sprintf($this->langTxt('er_delete'), $file);
    ...
    protected function setConn($mysql) {
        ...
        if ($this->conn_mod == 'pdo') {
            if (extension_loaded('PDO') === true)
                $this->conn_mod = 'pdo';
            else if (extension_loaded('mysqli') === true)
                $this->conn_mod = 'mysqli';
        }
        else if ($this->conn_mod == 'mysqli') {
            if (extension_loaded('mysqli') === true)
                $this->conn_mod = 'mysqli';
            else if (extension_loaded('PDO') === true)
                $this->conn_mod = 'pdo';
        }
        ...
        if ($this->conn_mod == 'pdo')
            ...
            $this->connMySQLi($mysql);
        else
            $this->setSqlError($this - langTxt('er_conn'));
    }
    ...
    private function sqlExec($sql) {
        if ($this->conn === false || $this->conn === NULL)
            $this->setConn($this->mysql);      // sets the connection to mysql
        ...
        if ($this->conn !== false) {
            ...
            if ($this->conn_mod == 'pdo') {
                ...
                if ($sqlre) {
                    if ($mode == 'select' || $mode == 'show')
                        $re = $this->getSelectMySQLi($sqlre);
                }
                else {
                    if (isset($this->conn->error_list[0]['error']))
                        $this->setSqlError($this->conn->error_list[0]['error']);
                    else
                        $this->setSqlError('Error');
                }
            }
        }
