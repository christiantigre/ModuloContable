...
//        $c = $this->conec_base();
            ...
            function b_generado($conn) {
                $date = date('Y-m-d');
                $year = date('Y');
                ?>
                <form name="BalancedeGenerado" id="BalancedeGenerado" action="ex_bresl.php" method="post">
                    <center>
                        ...
                        if ($resul_param->num_rows > 0) {
                            while ($clase_param = $resul_param->fetch_assoc()) {
                                $maxbalancedato = $clase_param['cont'];
                            }
                        } else {
                            echo "<script>alert('Ocurrio un error al cargar un parametro...')</script>";
                        }
                        ...
                        while ($row2 = mysqli_fetch_array($resulgrupos)) {
                            ...
                            echo '<tr>
                            <td>' . $row2['codcuenta'] . '</td>
                            <td>' . $row2['cuenta'] . '</td>';
                            echo '<td>' . number_format($row2['sum_deudor'], 2, '.', '') . '</td>';
                            echo '<td>' . number_format($row2['sum_acreedor'], 2, '.', '') . '</td>';
                            echo '</tr>';
                        }
                        echo '</table>';
                        ?>
