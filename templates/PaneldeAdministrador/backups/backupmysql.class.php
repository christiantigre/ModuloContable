    ...
    public function getListTables() {
        ...
        if (!$this->error) {
            ...
            $re['f'] = '<form action="' . $_SERVER['REQUEST_URI'] . '" method="post" id="frm_cht" name="frm_cht">'
                    ...
                    . '<label id="ch_all"><input onclick="check1()" id="todos"  type="checkbox">' . sprintf($this->langTxt('msg_select_all'), $nr) . '</label>'
            ...
            for ($i = 0; $i < $nr; $i++) {
                ...
                $re['f'] .= '<td><label><input type="checkbox" name="tables[]" value="' . $tables[$i] . '"> ' . $tables[$i] . ' </label></td></tr>';
            }
            $re['f'] .='</table></div>'
            ...
            if ($this->conn_mod == 'pdo') {
                ...
                if ($sqlre) {
                    ...
                    if (isset($this->conn->error_list[0]['error']))
                        ...
                        $this->setSqlError('Unable to execute the SQL query');
                }
            }
        }
