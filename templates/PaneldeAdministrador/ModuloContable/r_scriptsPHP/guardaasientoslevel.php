...
if ($result) {
    ...
    $sql_cont = "SELECT * FROM `num_asientos` WHERE `idnum_asientos` = '" . $asiento_num . "'";
    $result_as = mysqli_query($con, $sql_cont) or trigger_error("Query Failed! SQL: $sql_cont - Error: " . mysqli_error($con), E_USER_ERROR);
    if ($result_as) {
        while ($row_as = mysqli_fetch_assoc($result_as)) {
            $as = $row_as['t_ejercicio_idt_corrientes'];
        }
    }
    if ($as == '1') {
        print_r("Error!!!... Ya existe un balance inicial creado\n");
    } elseif ($as == "") {
        ...
        if ($maxbalancedato == '') {
            ...
            $insertasientoconcepto = "INSERT INTO `condata`.`num_asientos` (`idnum_asientos` ,`fecha` ,`concepto` , `t_bl_inicial_idt_bl_inicial`, `t_ejercicio_idt_corrientes`, mes,year)VALUES "
                    . "( '" . $incremento . "', '" . $datepick . "', '" . utf8_encode($concep) . "', '" . $maxbalancedato . "','" . $asiento_num . "','" . $month . "','" . $year . "');";
