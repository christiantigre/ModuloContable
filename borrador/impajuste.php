...
$sqlcargaconcepto = mysqli_query($db,"SELECT t_ejercicio_idt_corrientes as ej, concepto as c FROM `num_asientos_ajustes` WHERE `t_ejercicio_idt_corrientes`=".$idasientourl." ");
while ($row2 = mysqli_fetch_array($sqlcargaconcepto)) {
    $codgrupopdfas = $row2['ej']; 
    $concepto = $row2['c'];
}

$sql_transac = mysqli_query($db,"SELECT * FROM `ajustesejercicio` WHERE `t_bl_inicial_idt_bl_inicial` = '".$maxbalancedato."' AND ejercicio =".$codgrupopdfas." AND mes = '".$mes."' AND year = '".$year."' AND fecha = '".$feurl."' ");
