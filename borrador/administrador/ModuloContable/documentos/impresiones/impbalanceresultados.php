...
while ($productos2 = mysqli_fetch_array($sql_transac)) {
    ...
    while ($row2 = mysqli_fetch_array($sql_carga_grupos)) {
        ...
        $sql_carga_balance = mysqli_query($c, "SELECT `cod_cuenta` , `cuenta` , `t_bl_inicial_idt_bl_inicial` as b , 
                       `tipo` , `year` , `mes` , (
coalesce( debe_aj, 0 )
...
WHERE year = '" . $year . "' and tipo='" . $codigo_de_grupo . "'
GROUP BY cod_cuenta");
        while ($row3 = mysqli_fetch_array($sql_carga_balance)) {
            ...
            $pdf->Cell(250, 2, '    SUMAS DE : '.$row2['grupo'], '', 1, 'L');
