...
$result = mysqli_query($dbi, $consulta) or trigger_error("Query Failed! SQL: $consulta - Error: " . mysqli_error($dbi), E_USER_ERROR);
if ($result) {
...
$sqlbuscagrupos = "SELECT `idnum_asientos` as id,`t_ejercicio_idt_corrientes` ej,`concepto` c,fecha as f,"
        . "saldodebe as sald,saldohaber as salh FROM `num_asientos` "
        . "WHERE `t_bl_inicial_idt_bl_inicial`='" . $maxbalancedato . "' and"
        . " year ='" . $year . "' and `t_ejercicio_idt_corrientes` >1  order by ej desc";
